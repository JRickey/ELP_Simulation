...
def visualize_results():
    dt_prob = pd.read_csv(Path.cwd() / 'results' /'final_default_rate.csv')
    pp_prob = pd.read_csv(Path.cwd() / 'results' /'final_pp_rate.csv')

    data = pd.read_csv(Path.cwd() / 'loans' / 'test_data_final.csv')
    data = data.iloc[:100,:]
